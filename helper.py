...
runOutputUntilDone(process=ps, cmd="$ENV:onedrive", timeout=5, queue=q, done_message="done")

# Do whatever from here, below line is an example
link=(runOutputUntilDone(process=ps, cmd="$ENV:onedrive", timeout=5, queue=q, done_message="done"))

def getOneDrivePath():
    link=(runOutputUntilDone(process=ps, cmd="$ENV:onedrive", timeout=5, queue=q, done_message="done"))
    return (link[len(link)-1]).strip()